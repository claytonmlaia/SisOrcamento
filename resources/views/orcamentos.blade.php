                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="input-group">
                            <a onclick="aoClicarCadastrar()" href="" class="btn btn-primary"><i class="fas fa-plus"></i> NOVO ORÇAMENTO</a>
                        </div>
                        <br>
                        ...
                        <div class="input-group">
                            <table name="gradearquivos" class="busca-na-lista table table-striped col-md-12" cellspacing="0" cellpadding="0">
                                ...
                                @foreach($orcamentos as $orcamento)
                                    <tr>
                                        <td>{{ $orcamento->id }}</td>
                                        <td>@if($orcamento->clientes->cliente_tipo_id == 1){{ $orcamento->clientes->nome_cliente_responsavel }} @else{{ $orcamento->clientes->razao_social }}@endif</td>
                                        <td>
                                            @foreach($valororcamento as $valor)
                                                @if($valor->orcamento_id == $orcamento->id)
                                                @endif
                                            @endforeach
                                            R$ {{ number_format($valor->sum('itmValorFinal'), 2, ',', '.') }}
                                        </td>
                                        <td>
                                            <a onclick="aoClicarVisualizar({{ $orcamento->id }})"class="btn btn-xs btn-outline-info" href="" title="Visualizar"><i class="fas fa-eye"></i></a>
                                            <a onclick="aoClicarEditar({{ $orcamento->id }})" class="btn btn-xs btn-outline-primary" href="" title="Editar cliente"><i class="fas fa-edit editar"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
